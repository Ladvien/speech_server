...
class TTSRequest(BaseModel):
    ...
    voice_name: Optional[str] = Field(
        None, description="Voice name to use (for cloned voices)"
    )
    audio_prompt_path: Optional[str] = Field(
        None, description="Path to audio file for voice cloning"
    )
    speed: Optional[float] = Field(
        1.0, description="Speech speed multiplier", ge=0.1, le=3.0
    )
    exaggeration: Optional[float] = Field(
        0.5, description="Emotion exaggeration control", ge=0.0, le=2.0
    )
    cfg_weight: Optional[float] = Field(
        0.5, description="CFG weight for generation control", ge=0.0, le=1.0
    )
